...
let AddSubscriptionComponent = class AddSubscriptionComponent {
...
AddSubscriptionComponent.ctorParameters = () => [
...
let AdminComponent = class AdminComponent {
    ...
    get Users() {
        this.usersStorage.add(this.userService.getUsers((this.page - 1) * this.usersPerPage, this.usersPerPage).subscribe((users) => {
            this.users = users;
        }, err => {
            this.error = true;
        }));
        this.usersStorage.add(this.userService.countUsers().subscribe((count) => {
            this.count = count;
        }, err => {
            this.error = true;
        }));
...
let AuthorizationComponent = class AuthorizationComponent {
    ...
    authorization() {
        ...
        if (this.login != null && this.passwd != null) {
            // Create User for api
            const user = new _model_Users__WEBPACK_IMPORTED_MODULE_4__["Users"](null, null, this.login, this.passwd, null, null);
            this.authService.attemptAuth(this.login, this.passwd).subscribe(data => {
                //this.token.saveToken(data.token);
                //this.router.navigate(['user']);
                sessionStorage.setItem('username', this.login);
                let tokenStr = 'Bearer ' + data.token;
                sessionStorage.setItem('token', tokenStr);
                this.userService.authorizationUser(user).subscribe((id) => {
                    console.log("id:", id);
                    if (id === 0) {
                        this.userService.updateUser();
                        this.router.navigateByUrl('/admin');
...
let BillingaccountComponent = class BillingaccountComponent {
...
BillingaccountComponent.ctorParameters = () => [
    { type: _model_user_service__WEBPACK_IMPORTED_MODULE_3__["UserService"] }
];
...
let InformationComponent = class InformationComponent {
    ...
    get User() {
        this.userStorage.add(this.userService.getUser(this.userService.idUser).subscribe((user) => {
            this.user = user;
        }, err => {
            console.log(err);
            this.error = true;
...
let ManagementSubscriptionComponent = class ManagementSubscriptionComponent {
    constructor(userService) {
        this.userService = userService;
        this.periodOn = 1;
    }
...
ManagementSubscriptionComponent.ctorParameters = () => [
    { type: _model_user_service__WEBPACK_IMPORTED_MODULE_2__["UserService"] }
];
tslib__WEBPACK_IMPORTED_MODULE_0__["__decorate"]([
    Object(_angular_core__WEBPACK_IMPORTED_MODULE_1__["Input"])()
...
let ManagementUsersComponent = class ManagementUsersComponent {
    constructor(userService) {
        this.userService = userService;
    }
    ngOnInit() { }
...
ManagementUsersComponent.ctorParameters = () => [
    { type: _model_user_service__WEBPACK_IMPORTED_MODULE_2__["UserService"] }
];
tslib__WEBPACK_IMPORTED_MODULE_0__["__decorate"]([
    Object(_angular_core__WEBPACK_IMPORTED_MODULE_1__["Input"])()
...
let RegisterComponent = class RegisterComponent {
    ...
    Register(fName, lName, Login, passw, numCard, email) {
        ...
        const newUser = new _model_Users__WEBPACK_IMPORTED_MODULE_5__["Users"](this.firstName, this.lastName, this.newLogin, this.newPassword, this.numberCard, this.newEmail);
        this.userService.registerUser(newUser).subscribe((id) => {
            if (id === -1) {
                this.failedLogin = true;
            }
            else if (id > 0) {
                this.authService.attemptAuth(this.newLogin, this.newPassword).subscribe(data => {
                    //this.token.saveToken(data.token);
                    //this.router.navigate(['user']);
...
let SubscriptionComponent = class SubscriptionComponent {
    ...
    get products() {
        ...
        this.subscriptionStorage.add(this.userService.getUserSubscription(this.userService.idUser, (this.page - 1) * this.productPerPage, this.productPerPage, this.findSubscriptions).subscribe((subscriptions) => {
    ...
    get countProducts() {
        this.subscriptionStorage.add(this.userService.countSubscription(this.userService.idUser).subscribe((count) => {
            this.count = count;
        }, err => {
            this.error = true;
        }));
...
let ViewAllSubscriptionsComponent = class ViewAllSubscriptionsComponent {
    constructor(userService) {
        this.userService = userService;
    ...
    ngOnInit() {
        this.userService.updateSubscriptions();
        this.subscriptionStorage.add(this.userService.countSubscription(0).subscribe((count) => {
            this.count = count;
        }, err => {
            this.error = true;
        }));
    ...
    get products() {
        this.subscriptionStorage.add(this.userService.getUserSubscription(0, (this.page - 1) * this.productPerPage, this.productPerPage, "").subscribe((subscriptions) => {
            this.subscriptions = subscriptions;
        }, err => {
            this.error = true;
        }));
...
ViewAllSubscriptionsComponent.ctorParameters = () => [
    { type: _model_user_service__WEBPACK_IMPORTED_MODULE_3__["UserService"] }
];
...
let ViewSubscriptionAdminComponent = class ViewSubscriptionAdminComponent {
    ...
    get products() {
        this.subscriptionStorage.add(this.userService.getUserSubscription(this.id, (this.page - 1) * this.productPerPage, this.productPerPage, "").subscribe((subscriptions) => {
...
ViewSubscriptionAdminComponent.ctorParameters = () => [
    { type: _model_user_service__WEBPACK_IMPORTED_MODULE_3__["UserService"] },
...
let ChargingFirstGuard = class ChargingFirstGuard {
    constructor(router) {
        this.router = router;
...
ChargingFirstGuard.ctorParameters = () => [
    { type: _angular_router__WEBPACK_IMPORTED_MODULE_2__["Router"] }
];
...
class Users {
    constructor(firstName, lastName, login, password, numberCard, email) {
        this.numberCard = '';
...
let UserService = class UserService {
    ...
    getUserSubscription(id, offset, limit, name) {
    ...
    countUsers() {
        if (this.updateCountUsers) {
            ...
            this.httpClient.get('http://localhost:8080/api/users/count').subscribe((count) => {
    ...
    countSubscription(id) {
        if (this.updateCount) {
            this.subsID = new rxjs__WEBPACK_IMPORTED_MODULE_2__["ReplaySubject"](1);
            this.httpClient.get('http://localhost:8080/api/users/' + id + '/products/count').subscribe((count) => {
    ...
    authorizationUser(user) {
        this.subsID = new rxjs__WEBPACK_IMPORTED_MODULE_2__["ReplaySubject"](1);
        this.httpClient.get('http://localhost:8080/api/users/login/' + user.login).subscribe((id) => {
            this.idUser = id;
            this.subsID.next(id);
        }, err => {
    ...
    registerUser(user) {
        this.subsID = new rxjs__WEBPACK_IMPORTED_MODULE_2__["ReplaySubject"](1);
        this.httpClient.post('http://localhost:8080/api/users/up', user).subscribe((id) => {
            this.idUser = id;
            this.subsID.next(id);
        }, err => {
            this.subsID.error(err);
    ...
    transferMoney(user, id) {
        ...
        this.httpClient.post('http://localhost:8080/api/users/billing/' + id, user).subscribe((transfer) => {
    ...
    modifyUserSubscription(subscription) {
        this.httpClient.post('http://localhost:8080/api/users/' + this.idUser + '/product', subscription)
            .subscribe();
    }
    // ON/OFF subscription user_ID
    addSubscription(name, cost) {
        const newSubscription = new _Subscriptions__WEBPACK_IMPORTED_MODULE_4__["Subscriptions"]();
